const logger = (() => {
    ...
    //function to check if logs are enabled
    const isLogsEnabled = () => {

        //process.browser returns true when on client and return false when on server
        if(process.browser){
            const search = global?.window?.location?.search;
            const enabled = search && new URLSearchParams(search).get('debug') === 'true'
            
            //global objects are those objects that are available in all modules
            //creating areLogsEnabled variable in global objects
